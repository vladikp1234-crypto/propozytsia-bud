...
const initH = { region: "kyiv", area: 150, floors: 2, footprint: 0, floorHeight: 3.0, roomsCount: 3, bathrooms: 2, tier: "standart", budget: "h3", plot: 8,
  lawnType: "seed", lawnArea: 0, pathsArea: 0, treesFruit: 4, treesDecor: 3, treesBig: 0, hedgeLen: 0, flowerBeds: 0,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const labels = { area: "площа", footprint: "площа забудови", floorHeight: "висота поверху", plot: "ділянка", lawnType: "газон", lawnArea: "площа газону", pathsArea: "доріжки", treesFruit: "плодові дерева", treesDecor: "декоративні дерева", treesBig: "великомірні дерева", hedgeLen: "живопліт", flowerBeds: "квітники", roomsCount: "кількість кімнат", bathrooms: "санвузли", tier: "рівень оздоблення",
        ...
        {view === "form" && (() => {
          ...
          <div className="grid">
            <div style={{ display: "grid", gap: 16 }}>
              ...
              {stepId === "constr" && <div className="card"><div className="ch"><span className="cn">Конструктив</span><h2>Геометрія і з чого будуємо</h2></div>
                <div className="cb">
                  <div className="g2">
                    <label className="f">Площа забудови (слід будинку), м²
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <NumInput value={p.footprint || suggestFootprint(p)} def={suggestFootprint(p)} min={30} max={400} onChange={v => setP("footprint", v)} style={{ width: 90 }} />
                        <button className="tl" style={{ margin: 0 }} onClick={() => setP("footprint", 0)}>авто ≈ {suggestFootprint(p)}</button>
                      </div>
                      <span className="hint">Від сліду рахуються фундамент, покрівля, земляні та фасад — це найважливіший розмір</span></label>
                    <label className="f">Висота поверху, м
                      <NumInput value={p.floorHeight || 3.0} def={3.0} min={2.6} max={4.5} step={0.1} onChange={v => setP("floorHeight", v)} style={{ width: 90 }} />
                ...
                {mode === "house" && <div className="card"><div className="ch"><span className="cn">🌳</span><h2>Сад та озеленення</h2></div>
                  <div className="cb">
                    <label className="f">Газон
                      <div className="chips">
                        {[["none", "Без газону"], ["seed", "Посівний"], ["roll", "Рулонний"]].map(([k, n]) =>
                          <button key={k} className={"chip acc" + ((p.lawnType || "none") === k ? " on" : "")} onClick={() => setP("lawnType", k)}>{n}</button>)}
                      </div>
                      {p.lawnType !== "none" && <span className="hint">Площа газону авто: ділянка − слід будинку − доріжки ≈ {lawnA(p)} м² (можна змінити)</span>}</label>
                    {p.lawnType !== "none" && <div className="g2">
                      <label className="f">Площа газону, м²
                        <NumInput value={p.lawnArea || lawnA(p)} def={lawnA({ ...p, lawnArea: 0 })} min={0} max={3000} onChange={v => setP("lawnArea", v)} style={{ width: 100 }} /></label>
                      <label className="f">Доріжки/паркінг, м²
                        <NumInput value={p.pathsArea || Math.round((p.plot || 8) * 9)} def={Math.round((p.plot || 8) * 9)} min={0} max={800} onChange={v => setP("pathsArea", v)} style={{ width: 100 }} />
